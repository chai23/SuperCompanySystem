...
namespace 超好企業系統
{
    public partial class AllOrderSearchForm : Form
    {
        public AllOrderSearchForm()
        {
            InitializeComponent();
        }
        ...
        private void updateOrderListview()
        {
            ...
            string selectQuery2 = "SELECT `order`.*, `customer`.customer_name, `product`.is_join_reserve, `driver`.driver_name, `area`.area_name FROM `order` " +
                ...
                "LEFT JOIN `area` on `customer`.area_id = `area`.area_id " +
        ...
        private void AllOrderSearchForm_Resize(object sender, EventArgs e)
        {
